...
Go 分为**基本类型**、**复合类型**、**引用类型**、**接口类型**、**特殊类型**。一、基础基本类型
...
| `uint8`           | 0‑255（就是 byte）                |
| `int16`、`uint16` | 短整型                            |
| `int32`、`uint32` | rune 等价 int32，存 Unicode 字符  |
| `int64`、`uint64` | 长整型                            |
| `int`、`uint`     | 随系统 32/64 位自适应，开发最常用 |

> 注意：**不同 int 类型不能直接运算，必须显式转换**，没有隐式转换！

#### 浮点数

- `float32`、`float64`，默认字面量是`float64`
- 没有`float`简写
...
`[n]T`，**长度是数组类型一部分**，`[3]int` 和 `[5]int` 是两种不同类型，值拷贝。
...
最常用！**动态数组，引用视图**，底层指向数组。

- `len()`长度，`cap()`容量
- make 创建：`s := make([]int, 2,5)`
- append 追加，超出容量自动扩容
...
⚠️ 未 make 的 nil map 不能写，只能读。

### 4.struct 结构体

自定义聚合结构，类似 class，没有继承，只有组合。
...
> 引用类型变量存的是底层数据的地址，拷贝变量共享底层数据

- slice、map、channel

> ⚠️ 赋值只是复制引用，不是复制数据。想要完整复制需要手动拷贝。
...
- 接口存储：`(type, value)` 动态类型信息。

## 六、特殊类型

1. `rune`：等价`int32`，专门存放一个 Unicode 字符
2. `byte`：等价`uint8`，代表一个字节
3. `chan` 通道：goroutine 之间通信，`chan int`
4. `func` 函数类型：函数也是一等类型，可以赋值、传参

| 类型   | 底层    | 作用                        | 大小   |
| ------ | ------- | --------------------------- | ------ |
| `byte` | `uint8` | 原始字节、ASCII 字符        | 1 字节 |
| `rune` | `int32` | Unicode 字符（中文、emoji） | 4 字节 |

## 关键易错点总结（面试常考）

1. Go**没有隐式类型转换**，int→int32 必须手动转换
2. string 不可修改；slice/map 是引用；array 是值类型
3. nil：slice、map、chan、指针、接口可以为 nil；int、bool、string 零值不是 nil
4. 零值规则：变量声明不赋值自动给零值
   - 数字：0
   - bool：false
   - string：""
   - slice/map/chan/ 指针：nil
